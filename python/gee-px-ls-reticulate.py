...
def getVi(image):
    ...
    b1 = image.select(['SR_B1']).multiply(scalingFactor)
    b2 = image.select(['SR_B2']).multiply(scalingFactor)
    b3 = image.select(['SR_B3']).multiply(scalingFactor)
    b4 = image.select(['SR_B4']).multiply(scalingFactor)
    b5 = image.select(['SR_B5']).multiply(scalingFactor)
    b7 = image.select(['SR_B7']).multiply(scalingFactor)
    ...
    ndmi = image.normalizedDifference(['SR_B4', 'SR_B5'])
    image2 = (
...
def getVi8(image):
    ...
    b4 = image.select(['SR_B4']).multiply(scalingFactor)
    b5 = image.select(['SR_B5']).multiply(scalingFactor)
    # b6 = image.select(['SR_B6']).multiply(scalingFactor)
    b7 = image.select(['SR_B7']).multiply(scalingFactor)
